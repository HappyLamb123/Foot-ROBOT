...
/*   
  ...
  */ 
    ...
    double Kp_com = 0.0;
    double Kd_com = 0.0;
    double Kp_free = 100000.0;
    double Kd_free = 1000.0;
   ...
   // chrono::utils::PDSampler<float> sampler(2.4f * params.sphere_radius);
    ...
    std::string mesh_filename(GetChronoDataFile("granular/multi_hops/foot4.obj"));

    std::string mesh_filename_lid = GetChronoDataFile("granular/multi_hops/foot.obj");
...
*/
    ...
    double impact_vel = -20;
    double free_time = -impact_vel / 100.0 / 9.81 * 2;
    for (int i = 5; i < 6; i++)
    {
       ...
       // std::cout<<body_mass << std::endl;
        for (double t = 0; t < (double)params.time_end; t += iteration_step, curr_step++) {
            ...
            if (t >= particle_moving && plate_impact_state == false) {
                ...
                rigid_plate->SetPos_dt(ChVector<>(0, 0, impact_vel));
                ...
                rigid_body->SetPos_dt(ChVector<>(0, 0, impact_vel));
...
uy=0;
            ...
            else if (t > prepare_time + time_array[M - 1] && t < prepare_time + time_array[M - 1] + free_time) {
                delta_q = 25 - (rigid_body->GetPos()[2] - rigid_plate->GetPos()[2]);
                delta_qdot = 0 - (rigid_body->GetPos_dt()[2] - rigid_plate->GetPos_dt()[2]);
                ux = 0;
                uy = (delta_q * Kp_free + delta_qdot * Kd_free) * F_SI_TO_CGS / 100;
                utheta = 0.0;
            ...
            if (counter % 4 == 0 && t > prepare_time && t < prepare_time + time_array[M - 1]+free_time){
