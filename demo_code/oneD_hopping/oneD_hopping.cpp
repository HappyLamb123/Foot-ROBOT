...
int main(int argc, char* argv[]) {
   ...
   // chrono::utils::PDSampler<float> sampler(2.4f * params.sphere_radius);
    ...
    std::string mesh_filename(GetChronoDataFile("granular/oneD_hopping/foot.obj"));

    std::string mesh_filename_lid = GetChronoDataFile("granular/oneD_hopping/foot.obj");
  ...
  //   std::vector<float3> mesh_translations(1, make_float3(-20.f, -20.f, 0.f));
    ...
    float plate_density = 20.0;//params.sphere_density / 100.f;
    ...
    for (int i = 5; i < 6; i++)
    {
        ...
        for (double t = 0; t < (double)params.time_end; t += iteration_step, curr_step++) {
            ...
            if (t >= particle_moving && plate_impact_state == false) {
                ...
                rigid_plate->SetPos(ChVector<>(0, 0, max_z-0.1));
